  ...
  try{
    ...
    if(!window.pdfjsLib){
      await new Promise((res,rej)=>{
        ...
        s.onload=res; s.onerror=()=>rej(new Error("Could not load PDF library. Check your internet connection."));
    ...
    if(!text.trim()) throw new Error("PDF appears to be empty or image-only (no extractable text)");
    return text;
  }catch(e){
...
async function extractDOCX(file){
  try{
    ...
    throw new Error("Could not extract text from DOCX: "+e.message);
  }
}
...
const GEMINI_KEYS=[
  "AIzaSyCsmzwS4FdnavHDG2RvUCubHl8GTj7jCao",
  "AIzaSyCGeOFLQ9c_99rcj6yy6nMyv02wVqVTrwU",
];
...
async function callGemini(prompt){
  ...
  throw new Error(lastErr?.message||"All Gemini keys failed");
}
...
/* ─── AI: parse + score a single resume (Gemini, retry once) ────────────── */
async function analyzeResume(resumeText, jobTitle, jobDescription, jobSkills, jobExperience, jobEducation){
  const prompt=`You are an expert HR AI. Analyze this resume against the job requirements and return ONLY valid JSON (no markdown, no explanation, no text outside the JSON object).
...
matchScore must be 0-100 integer. scoreBreakdown values must each be 0-100 integers.`;
  ...
  for(let attempt=0;attempt<2;attempt++){
    try{
      ...
      // Sanitise all fields so downstream code never breaks
      return {
      ...
      if(attempt===0) await new Promise(r=>setTimeout(r,700)); // small delay before retry
    }
  }
